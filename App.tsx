...
export default function App() {
  ...
  const playAt = async (i) => { setCurrentIndex(i); setVideoUri(playlist[i].uri); };
  return (
    <View style={s.bg}>
      <StatusBar hidden />
      <Text style={s.title}>NexPlayer</Text>
      {videoUri ? (
        <Video ref={videoRef} source={{uri:videoUri}} style={s.video} resizeMode={ResizeMode.CONTAIN} volume={volume} shouldPlay={false} useNativeControls onPlaybackStatusUpdate={(st)=>{ if(st.isLoaded){setPosition(st.positionMillis);setDuration(st.durationMillis||1);setIsPlaying(st.isPlaying);}}} />
      ) : (
      ...
      {videoUri && <>
        ...
        <View style={s.row}>{[0.5,0.75,1.0,1.25,1.5,2.0].map(sp=>(<TouchableOpacity key={sp} onPress={()=>changeSpeed(sp)} style={[s.chip,speed===sp&&s.on]}><Text style={s.chipTxt}>{sp}x</Text></TouchableOpacity>))}</View>
        <Text style={s.label}>Volume</Text>
        <View style={s.row}>{[0.25,0.5,0.75,1.0].map(v=>(<TouchableOpacity key={v} onPress={()=>setVolume(v)} style={[s.chip,volume===v&&s.on]}><Text style={s.chipTxt}>{Math.round(v*100)}%</Text></TouchableOpacity>))}</View>
        <TouchableOpacity style={s.addBtn} onPress={pickVideo}><Text style={s.addTxt}>+ Video Add</Text></TouchableOpacity>
        <ScrollView style={s.list}>{playlist.map((f,i)=>(<TouchableOpacity key={i} onPress={()=>playAt(i)} style={[s.item,i===currentIndex&&s.itemOn]}><Text style={s.itemTxt} numberOfLines={1}>{f.name}</Text></TouchableOpacity>))}</ScrollView>
      </>}
    </View>
  );
}
const s = StyleSheet.create({
  ...
  boxIcon:{fontSize:48},boxTxt:{color:'#FF6B00',fontSize:15,marginTop:8},
