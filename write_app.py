    ...
    <View style={{ flex: 1, backgroundColor: "#000" }}>
      ...
      {showCtrl && !locked && (
        <View style={p.overlay}>
          ...
          {/* Bottom controls */}
          <View style={p.botGrad}>
            {/* Progress bar */}
            <View style={p.progRow}>
              <Text style={p.timeLeft}>{fmt(pos)}</Text>
              <TouchableOpacity style={p.progTrack} onPress={e => videoRef.current?.setPositionAsync((e.nativeEvent.locationX / (W - 96)) * dur)}>
                <View style={p.progBg}>
                  <View style={[p.progFill, { width: pct + "%" }]} />
                  <View style={[p.progDot, { left: pct + "%" }]} />
                </View>
              </TouchableOpacity>
              <Text style={p.timeRight}>{fmt(dur)}</Text>
            </View>
            {/* Bottom buttons */}
            <View style={p.botBtns}>
              <TouchableOpacity onPress={() => setShowSpeed(true)} style={p.botChip}>
                <Text style={p.botChipTxt}>⚡ {speed}x</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={() => setLoop(!loop)} style={p.botChip}>
                <Text style={p.botChipTxt}>{loop ? "🔂" : "🔁"} Loop</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={pickVideo} style={p.botChip}>
                <Text style={p.botChipTxt}>＋ Add</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
      ...
      {locked && (
        <TouchableOpacity style={p.lockScreen} onPress={() => setLocked(false)}>
          <View style={p.lockBox}>
            <Text style={{ fontSize: 32 }}>🔒</Text>
            <Text style={{ color: "#fff", marginTop: 8 }}>Tap to unlock</Text>
          </View>
        </TouchableOpacity>
      )}

      <Modal visible={showSpeed} transparent animationType="slide">
        <TouchableOpacity style={p.modalBg} onPress={() => setShowSpeed(false)}>
          <View style={p.speedSheet}>
            <View style={p.sheetHandle} />
            <Text style={p.sheetTitle}>Playback Speed</Text>
            {SPEEDS.map(sp => (
              <TouchableOpacity key={sp} style={[p.speedRow, speed === sp && p.speedActive]} onPress={() => { setSpeed(sp); videoRef.current?.setRateAsync(sp, true); setShowSpeed(false); }}>
                <Text style={[p.speedTxt, speed === sp && { color: ACCENT, fontWeight: "800" }]}>{sp === 1.0 ? "Normal" : sp + "x"}</Text>
                {speed === sp && <Text style={{ color: ACCENT, fontSize: 18 }}>✓</Text>}
              </TouchableOpacity>
            ))}
          </View>
        </TouchableOpacity>
